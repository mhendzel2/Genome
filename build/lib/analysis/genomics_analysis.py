...
class GenomicsAnalyzer:
    ...
    def differential_expression(self, uploaded_files: Dict[str, Any], **params) -> pd.DataFrame:
        """Perform differential expression analysis and return a DataFrame."""
        if not uploaded_files:
            return pd.DataFrame()

        # In a real scenario, this would handle control/treatment groups.
        # For now, we pass all files to the R integration.
        json_output = self.r_integration.differential_expression(uploaded_files, **params)

        if isinstance(json_output, str) and json_output.strip():
            try:
                # The R script now returns a JSON representation of the data frame
                results_df = pd.read_json(json_output)

                # Optional: Add adjusted p-value if not present
                if 'pvalue' in results_df.columns and 'padj' not in results_df.columns:
                    pvals = results_df['pvalue'].dropna()
                    if not pvals.empty:
                        _, padj = fdrcorrection(pvals)
                        results_df['padj'] = pd.Series(padj, index=pvals.index)

                return results_df
            except (ValueError, TypeError) as e:
                print(f"Error parsing JSON from R script: {e}")
                return pd.DataFrame()
    ...
    def chromatin_interaction_analysis(self, uploaded_files: Dict[str, Any], **params) -> pd.DataFrame:
        """
        Parses chromatin interaction data in BEDPE format.
        Assumes the first file is the one to be analyzed.
        """
        if not uploaded_files:
            return pd.DataFrame()
        ...
        file_info = uploaded_files[first_file_key]

        try:
            file_info['file'].seek(0)
            # Define column names for a standard BEDPE file
            columns = [
                'chrom1', 'start1', 'end1',
                'chrom2', 'start2', 'end2',
                'name', 'score', 'strand1', 'strand2'
            ]
            # Read the file, assuming tab-separated and no header
            df = pd.read_csv(file_info['file'], sep='\t', header=None, comment='#')
            # Assign column names, trimming to the number of columns present
            df.columns = columns[:len(df.columns)]
            return df

        except Exception as e:
            print(f"Error processing Hi-C file {file_info['file'].name}: {e}")
            return pd.DataFrame()

    def multi_omics_integration(self, uploaded_files: Dict[str, Any], correlation_threshold: float = 0.7) -> pd.DataFrame:
        """
        Integrate multiple omics data types by finding correlations.
        Assumes uploaded files are tabular data with a common index column (e.g., gene ID).
        """
        ...
        for file_info in uploaded_files.values():
            try:
                ...
                df = pd.read_csv(file_info['file'], index_col=0)
                data_frames.append(df)
            except Exception as e:
                print(f"Error reading file {file_info['file'].name}: {e}")
                continue
        ...
        # Transform the correlation matrix into a list of links for the network graph
        corr_links = corr_matrix.stack().reset_index()
        corr_links.columns = ['source', 'target', 'weight']

        # Filter out self-loops and weak correlations
        strong_links = corr_links[
            (corr_links['source'] != corr_links['target']) &
            (abs(corr_links['weight']) > correlation_threshold)
        ]
